...
void BMP280::initialize(uint8_t t_sb, uint8_t osrs_p, uint8_t filter)
{
    char cmd[18];
    ...
    ctrl_meas.val = 0b01010111; // Temparature oversampling x2 010, Pressure oversampling x16 101, Normal mode 11
    ctrl_meas._.osrs_p = osrs_p;
    cmd[1] = ctrl_meas.val;
    i2c.write(address, cmd, 2);
 
    cmd[0] = 0xf5; // config
    config.val = 0b10111100; // Standby 1000ms, Filter x16
    config._.t_sb = t_sb;
    config._.filter = filter;
    cmd[1] = config.val;
    i2c.write(address, cmd, 2);
