...
void connect(void) {
    c++;
    if (c >= 2 && EtatN != null && EtatN != Alarme && EtatN != Eteint && EtatN != Perdu) {
        if(EtatN == Armed ){
            EtatN = Perdu;
            perte = Oui;
            TMR1_WriteTimer(0);
        ...
        do { LATBbits.LATB1 = 1; } while(0);
        do { LATBbits.LATB2 = 1; } while(0);
    } else if (EtatN == Eteint && connexion == 0) c = 0;
...
void affiche(void) {
    ...
    printf("X = %02X %02X", trame[12], trame[13]);

    LCD_Function(curseur, 2, 0);
    printf("Y = %02X %02X", trame[14], trame[15]);
