...
double rtd_lookup(double x, int size) {
    ...
    if (i == size)return (table[i - 1].temperature);
    if (i == 0) return (table[i].temperature);
...
void read_ADC(void) {
    ...
    adc = ADC_GetConversion(channel_AN0);
    _delay((unsigned long)((250)*(20000000/4000.0)));
    tensionADC = (((double) (adc)*2) / 4095);
    volt = (tensionADC + 3) / 33.78;
    res = volt / 0.001;

    temp = rtd_lookup((double) res, 151);
