...
void main(void) {
    ...
    TMR0_SetInterruptHandler(interruption);
    ...
    while (1) {
        ...
        if (udemarrage[1] == 1){
            ...
            txMessage.frame.data0 = (uint8_t)(uT_vehicule_CAN);
            txMessage.frame.data1 = (uint8_t)(uT_vehicule_CAN >>8);
        ...
        if(itemps == 6)
            ...
            txMessage.frame.dlc = 2;
            txMessage.frame.data0 = (uint8_t)(uT_consigne_CAN);
            txMessage.frame.data1 = (uint8_t)(uT_consigne_CAN >> 8);
            CAN_transmit(&txMessage);
        }
        if(itemps == 12)
        {
...
float temperature(uint16_t uibrute) {
    float ftemperature;
    float fcelsius;
    ftemperature = (uibrute / 4095.0) * 500;
    fcelsius = (ftemperature - 250);
    return fcelsius;
}
void controle_seuil(void) {
    fvariation = (fT_consigne - fT_vehicule);
    if ((fvariation < 0.5) && (fvariation > -0.5))
    {
        do { LATBbits.LATB6 = 0; } while(0);
        mode statue = eteint;
        do { LATBbits.LATB7 = 0; } while(0);
