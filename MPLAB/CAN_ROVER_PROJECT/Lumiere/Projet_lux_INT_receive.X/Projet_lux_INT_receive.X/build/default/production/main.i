...
# 116 "./mcc_generated_files/ecan.h"
void ECAN_Initialize(void);
# 138 "./mcc_generated_files/ecan.h"
void CAN_sleep(void);
# 159 "./mcc_generated_files/ecan.h"
uint8_t CAN_transmit(uCAN_MSG *tempCanMsg);
# 183 "./mcc_generated_files/ecan.h"
uint8_t CAN_receive(uCAN_MSG *tempCanMsg);
# 205 "./mcc_generated_files/ecan.h"
uint8_t CAN_messagesInBuffer(void);
# 229 "./mcc_generated_files/ecan.h"
uint8_t CAN_isBusOff(void);
# 253 "./mcc_generated_files/ecan.h"
uint8_t CAN_isRXErrorPassive(void);
# 277 "./mcc_generated_files/ecan.h"
uint8_t CAN_isTXErrorPassive(void);
# 317 "./mcc_generated_files/ecan.h"
void ECAN_SetRXB0InterruptHandler(void (*handler)(void));
# 332 "./mcc_generated_files/ecan.h"
void ECAN_RXB0I_ISR(void);
# 372 "./mcc_generated_files/ecan.h"
void ECAN_SetWakeUpInterruptHandler(void (*handler)(void));
# 387 "./mcc_generated_files/ecan.h"
...
void main(void) {
    ...
    TMR2_SetInterruptHandler(gestion_compteur);
    TMR1_SetInterruptHandler(compteurTT);
    TMR0_SetInterruptHandler(actif);
    ...
    while (1) {
        switch (compteurTTCAN) {
            case 16:
                ...
                if (compteur >= 10) {
                    erreur_adc();
                    (CAN_transmit(&Lux_INT_Tx));
                    compteur = 0;
                }

                if (Lux_INT_Rx.frame.id == 0x61) {
                    if (Lux_INT_Rx.frame.dlc == 0)
        ...
        if (!clef) {
            ...
            switch (Lux_INT_Rx.frame.id) {
                ...
                case 0x13:
                    ...
                    if (Lux_INT_Rx.frame.data0 == 0x00) {
                        porte_UnLock = 1;
                    } else {
                        porte_UnLock = 0;
                    }
...
void erreur_adc(void) {
    do { LATAbits.LATA2 = 1; } while(0);
    erreur_0 = ADC_GetConversion(0);
    do { LATAbits.LATA2 = 0; } while(0);

    do { LATAbits.LATA3 = 1; } while(0);
    erreur_1 = ADC_GetConversion(1);
    do { LATAbits.LATA3 = 0; } while(0);

    printf("Erreurs: %4d %4d\r\n\n", erreur_0, erreur_1);

    if (erreur_0 <= 1638 || erreur_0 >= 1966)
    {
        Lux_INT_Tx.frame.data0 = 0x01;
        printf("Etat lumiere avant : ERREUR \n\r");
    } else {
        Lux_INT_Tx.frame.data0 = 0x00;
        printf("Etat lumiere avant: ACTIVE \n\r");
    }

    if (erreur_1 < 1638 || erreur_1 > 1966)
    {
        Lux_INT_Tx.frame.data1 = 0x01;
        printf("Etat lumiere arriere: ERREUR \n\r");
    } else {
        Lux_INT_Tx.frame.data1 = 0x00;
